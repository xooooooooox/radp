...
start_app() {
	...
	docker-compose --env-file "$app_dir"/.env --env-file "$app_dir"/.env.app."$app_name" -f "$app_dir"/docker-compose-"$app_name".yaml up -d --pull always
	popd || return 1
}
...
start_infr() {
	...
	docker-compose -f "$infra_dir"/docker-compose-"$infra_name".yaml up -d
	popd || return 1
}
...
main() {
	...
	if (( "${#failed_names[@]}" > 0 ));then
		echo "[$type]==>ERROR: failed to startup ${failed_names[*]}"
	fi
}
