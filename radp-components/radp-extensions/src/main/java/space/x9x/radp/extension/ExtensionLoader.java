...
public class ExtensionLoader<T> {
    ...
    /**
     * 获取指定类型的扩展加载器
     * 该方法用于获取一个扩展加载器实例，该实例负责根据指定类型（接口）加载其实现类
    ...
    /**
     * 检查扩展类型是否符合规范
     * 此方法用于确保传入的扩展类型（接口）符合特定的规则：非空、为接口类型且带有SPI注解
    ...
    private static <T> void checkExtensionAnnotation(Class<T> type) {
        ...
        if (!type.isAnnotationPresent(SPI.class)) {
            throw new IllegalArgumentException("Extension type(" + type + ") is not extension, because it is not annotated with @" + SPI.class.getSimpleName());
        }
    }
    ...
    private void injectValue(T instance, Method method, Class<?> parameterType, String property) {
        ...
        if (object != null) {
            try {
                ...
                log.error("Failed to inject via method {} of interface {}: {}", method.getName(), parameterType.getName(), e.getMessage(), e);
            }
        }
    }
    ...
    private void loadDirectory(Map<String, Class<?>> extensionClasses,
        ...
        try {
            ...
            log.error("Exception occurred when loading extension class (interface: {}, description file: {}).", type, fileName, e);
        }
    }
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.isEmpty()) {
                    try {
                        ...
                        IllegalStateException illegalStateException = new IllegalStateException("Failed to load extension class (interface: " + type + ", class line: " + line + ") in " + resourceURL + ", cause: " + e.getMessage(), e);
    ...
    private void loadClass(Map<String, Class<?>> extensionClasses,
        ...
        if (!type.isAssignableFrom(clazz)) {
            throw new IllegalArgumentException("Error occurred when loading extension class (interface: " +
    ...
    /**
     * 保存扩展类到扩展类映射中
     * 如果存在相同名称的扩展类，默认情况下优先使用已存在的类除非允许覆盖
     * 如果两个类都有Order注解，优先使用Order值较小的类
     *
        ...
        if (c == null || overridden) {
            ...
            if (clazz.isAnnotationPresent(Order.class) || c.isAnnotationPresent(Order.class)) {
                ...
                if (srcValue > destValue) {
                    log.debug("Compare extension {} name {} use {} instead of {}", type.getName(), name, clazz.getName(), c.getName());
                    extensionClasses.put(name, clazz);
                }
                // 否则，优先使用已存在类
                log.debug("Compare extension {} name {} use {} instead of {}", type.getName(), name, c.getName(), clazz.getName());
                return;
            }
