...
public class AdaptiveExtensionLoader<T> {
    ...
    /**
     * 缓存适应性类
     * 该方法用于缓存一个适应性类，如果已经缓存了一个不同的适应性类，则抛出异常
     *
     * @param clazz      要缓存的适应性类
     * @param overridden 是否覆盖已缓存的适应性类
     *                   如果设为true，允许覆盖当前缓存的适应性类；
     *                   如果设为false且cachedAdaptiveClass不为空且不等于clazz，
    ...
    public void cachedAdaptiveClass(Class<?> clazz, boolean overridden) {
        ...
        if (cachedAdaptiveClass == null || overridden) {
            ...
            throw new IllegalStateException("More than one adaptive class found: "
    ...
    private Class<?> createAdaptiveExtensionClass() {
        // 生成自适应扩展类的字节码
        String code = new AdaptiveClassCodeGenerator(extensionLoader.getType(), extensionLoader.getCachedDefaultName()).generate();
        // 获取类加载器
