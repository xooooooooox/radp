...
public class DubboExceptionFilter implements Filter, BaseFilter.Listener {
	 ...
	 * <p>
	 * 较低的值表示更高的优先级。值-1确保此过滤器 在过滤器链中较早运行，以便正确处理异常.
	 */
	 ...
	 * <p>
	 * 此构造函数初始化过滤器并记录一条消息， 表明Dubbo3异常过滤器已初始化.
	 */
	 ...
	 * 执行服务调用
	 * <p>
	 * 该方法在服务调用前被调用，本过滤器不修改调用过程， 仅将调用传递给下一个过滤器或最终的服务实现.
	 * @param invoker 调用者
	 ...
	 * 处理服务响应.
	 * <p>
	 * 该方法在服务提供者返回响应后被调用，用于处理响应中的异常。 根据异常类型和上下文决定是直接抛出异常还是包装后再抛出
	...
	public void onResponse(Result appResponse, Invoker<?> invoker, Invocation invocation) {
		...
		if (appResponse.hasException() && GenericService.class != invoker.getInterface()) {
			try {
				...
				// 处理异常过滤器本身出现的异常
				logFilterException(ex, invoker, invocation);
			}
		}
	}
	 ...
	 * 判断异常是否应该直接抛出
	 * <p>
	 * 以下情况的异常会直接抛出： 1. 检查型异常（非RuntimeException的Exception） 2. 在方法签名中声明的异常 3.
	 * 与接口在同一个JAR文件中的异常 4. JDK内置异常（java.* 或 javax.*包中的异常） 5. 自定义基础异常（BaseException） 6.
	 * RPC异常（RpcException）
	...
	private void logUndeclaredException(Throwable exception, Invoker<?> invoker, Invocation invocation) {
		log.error("捕获到未在方法签名中声明的异常，调用方: {}, 服务: {}, 方法: {}, 异常类型: {}: {}",
				RpcContext.getServerContext().getRemoteHost(), invoker.getInterface().getName(),
	...
	private void logFilterException(Throwable e, Invoker<?> invoker, Invocation invocation) {
		log.warn("异常过滤器处理失败，调用方: {}, 服务: {}, 方法: {}, 异常类型: {}: {}", RpcContext.getServerContext().getRemoteHost(),
	 ...
	 * 处理调用过程中发生的错误.
	 * <p>
	 * 该方法在调用过程中发生错误时被调用，用于记录错误信息。 与onResponse不同，onError处理的是在RPC调用过程中发生的错误，
	 * 而不是服务执行过程中抛出的业务异常。
	 * @param t 错误对象
	 * @param invoker 调用者
	 * @param invocation 调用信息
	...
	public void onError(Throwable t, Invoker<?> invoker, Invocation invocation) {
		log.error("调用过程中发生未捕获的异常，调用方: {}, 服务: {}, 方法: {}, 异常类型: {}: {}", RpcContext.getServerContext().getRemoteHost(),
				invoker.getInterface().getName(), invocation.getMethodName(), t.getClass().getName(), t.getMessage(),
				t);
	}
