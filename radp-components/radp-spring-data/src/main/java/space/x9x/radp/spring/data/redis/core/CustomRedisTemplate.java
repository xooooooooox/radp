...
public interface CustomRedisTemplate {
	...
	/**
	 * Default expiration time (one day) in seconds. This constant defines the default
	...
	/**
	 * Retrieves an object from the cache by its key and deserializes it to the specified
	 * class type.
	...
	/**
	 * Retrieves a List of objects from the cache by its key and deserializes them to the
	 * specified class type.
	 * @param <T> the type of the objects to be returned
	 * @param key the key to look up in the cache
	 * @param clazz the class of the objects to be returned
	 * @return an Optional containing the List of objects if found, otherwise an empty
	 * Optional
	 */
	<T> Optional<List<T>> getForList(String key, Class<T> clazz);

	/**
	 * Sets a value in the cache with the specified key. The value will be stored with the
	 ...
	 * @param key the key of the hash map in the cache
	 * @return an Optional containing a Map of all the key-value pairs if found, otherwise
	 * an empty Optional
	 */
	...
	/**
	 * Deletes one or more hash keys from the hash stored at the specified key in the
	 * cache.
	 * @param key the key of the hash map in the cache
	...
	/**
	 * Retrieves a sublist of elements from a list stored in the cache, within the
	 * specified range, and deserializes them to the specified class type.
	 * @param <T> the type of the objects to be returned
	 * @param key the key to look up in the cache
	...
	/**
	 * Pushes all elements of the provided list to the right end of the list stored at the
	 ...
	 * @param key the key to look up in the cache
	 * @return an Optional containing the Set of members if found, otherwise an empty
	 * Optional
	 */
	Optional<Set<String>> members(String key);

	/**
	 * Adds a list of values associated with the given key in the cache. This method
	 * stores the provided list of values in the cache using the specified key.
	...
	default <T> String buildRedisKey(String keyPrefix, T key) {
		return StringUtil.join(keyPrefix, Strings.COLON, String.valueOf(key));
	}
