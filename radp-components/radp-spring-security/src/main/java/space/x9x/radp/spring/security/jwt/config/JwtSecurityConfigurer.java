...
public class JwtSecurityConfigurer {
	...
	public void configure(HttpSecurity httpSecurity,
		...
		if (this.jwtAuthorizationFilter != null) {
			...
			httpSecurity
				// 使用默认 login 页面和 loginProcessingUrl("/login")
		...
		httpSecurity
			// 设置每个请求的权限
			// Spring Security 5.x 使用 antMatchers
			// Spring Security 6.x 使用 authorizeHttpRequests + requestMatchers
			.authorizeHttpRequests(registry -> {
				// 1. 全局共享规则
				// 1.1 认证: 对于显式配置需要认证的 url, 必须认证
				registry.requestMatchers(authenticatedUrls).authenticated();
				// 1.2 放行: 允许对网站 静态资源 的无授权访问
				registry.requestMatchers(HttpMethod.GET, "/*.css", "/*.js", "/*.html").permitAll();
				// 1.3 放行: 对配置文件中指定的 permit-all-urls 接口允许未授权访问(比如登录/注册等)
				registry.requestMatchers(permittedUrls).permitAll();
				// 1.4 放行: 对 @PermitAll 标注的接口允许未授权访问
				registry.requestMatchers(permitAllUrlsFromAnnotations).permitAll();
				// 1.5 放行: 对配置文件中指定的 anonymous-urls 接口允许未授权访问
				registry.requestMatchers(anonymousUrls).permitAll();
