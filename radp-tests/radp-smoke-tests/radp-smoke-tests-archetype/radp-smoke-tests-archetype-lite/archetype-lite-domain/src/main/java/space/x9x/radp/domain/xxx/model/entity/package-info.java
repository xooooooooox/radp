 ...
 * <h4>概念</h4>
 * <p>
 * <strong>实体=唯一标识+状态属性+行为动作（功能）</strong>，是 DDD 中的一个基本构建块，它代表了具有唯一标识的领域对象。实体不仅仅包含数据（状态属性），
 * 还包含了相关的行为（功能），并且它的标识在整个生命周期中保持不变。
 * </p>
 *
 * <h4>特征</h4>
 * <ul>
 * <li><strong>唯一标识：</strong>实体具有一个可以区分其它实体的唯一标识符。这个标识符可以是一个
 * ID，一个复合键或者是一个自然键，关键是它能够唯一的标识实体实例。</li>
 * <li><strong>领域标识：</strong>实体的标识通常来源于业务领域，例如用户ID、订单ID 等。这些标识符在业务上有特定的含义，并且在系统中是唯一的。</li>
 * <li><strong>委派标识：</strong>在某些情况下，实体的标识可能是由
 * ORM（对象关系映射）框架自动生成的，如数据库中的自增主键。这种标识符虽然可以唯一标识实体，但它并不直接来源于业务领域。</li>
 * </ul>
 *
 * <h4>用途</h4>
 * <ul>
