 ...
 * 例如为实体对象提供属性值的描述，如一个公司雇员的级别值对象，或一个下单商品的四级地址信息对象。 因此，在开发值对象时，通常不会提供 setter 方法，而是通过构造函数或
 * Builder 方法来实例化对象。 这个对象通常不会独立作为方法的入参对象，但可以独立作为出参对象使用。
 * <p>
 * <h3>概念</h3>
 * <p>
 * <strong>值对象</strong>是由一组属性组成的，它们共同描述了一个领域概念。与实体（Entity）不同，值对象不需要有一个唯一的标识符来区分它们。
 ...
 * <ul>
 * <li><strong>不可变性
 * (Immutability)：</strong>值对象一旦被创建，其状态就不应该发生变化。这有助于保证领域模型的一致性和线程安全性。</li>
 * <li><strong>等价性
 * (Equality)：</strong>值对象的等价性不是基于身份或引用，而是基于对象的属性值。如果两个值对象的所有属性值都相等，那么这两个对象就被认为是等价的。</li>
 * <li><strong>替换性
 * (Replace-ability)：</strong>由于值对象是不可变的，任何需要改变值对象的操作都会导致创建一个新的值对象实例，而不是修改现有的实例。</li>
